...
# https://github.com/timescale/pgai/issues/728 # Known issue not the batching logic... added an Overhead to try mitigate
...
def get_measures_from_embeddings(all_embeddings, binary_flags):
    ...
    for flag in binary_flags:
        ...
        cos_distances = [
            ...
            cosine_distance(search_intent_embedding, readme_embedding)
        ]
        ...
        euclidean_distances = [
            ...
            euclidean_distance(search_intent_embedding, readme_embedding)
        ]
        ...
        l1_distances = [
            ...
            l1_distance(search_intent_embedding, readme_embedding)
        ]
        ...
        # Differences
        differences = difference_vector(search_intent_embedding, name_embedding).tolist() + difference_vector(search_intent_embedding, snippet_embedding).tolist() + difference_vector(search_intent_embedding, readme_embedding).tolist()

        difference_vectors.append(differences)

        # Overlap / Product
        overlap_products = overlap_product_vector(search_intent_embedding, name_embedding).tolist() + overlap_product_vector(search_intent_embedding, snippet_embedding).tolist() + overlap_product_vector(search_intent_embedding, readme_embedding).tolist()
...
def prepare_infos_testing_and_training(the_dataset, dimensions, embedding_model, tokenizer_model):
    ...
    for repo_infos in the_dataset:
        texts_to_embed.append(safe_non_empty_string_field_access("search_intent", repo_infos))
        texts_to_embed.append(safe_non_empty_string_field_access("name", repo_infos))
        texts_to_embed.append(safe_non_empty_string_field_access("snippet", repo_infos))
        # texts_to_embed.append(clean_markdown(safe_non_empty_string_field_access("readme", repo_infos)))
        texts_to_embed.append(safe_non_empty_string_field_access("readme", repo_infos))

        relevance_grade = repo_infos["grade"]
...
def llm_binary_decide(search_intent: str, name: str, snippet: str, readme: str, previous_decision: float) -> bool:
    """
...
Your only job is to decide whether a repository is relevant
...
Determine whether the repository appears relevant to the intent.
...
def test_gpt_4o_perf(the_test_dataset, nbr_iter = 5):
    ...
    for _ in range(nbr_iter):
        ...
        for repo_infos in the_test_dataset:
            list_decisions.append(llm_binary_decide(
                search_intent=safe_non_empty_string_field_access("search_intent", repo_infos),
                name=safe_non_empty_string_field_access("name", repo_infos),
                snippet=safe_non_empty_string_field_access("snippet", repo_infos),
                # readme=clean_markdown(safe_non_empty_string_field_access("readme", repo_infos)),
                readme=safe_non_empty_string_field_access("readme", repo_infos),
...
def run_models_training_testing_pipeline_github_repos(train_dataset, test_dataset, add_to_folder_name):

    root_folder = f"{str(add_to_folder_name).strip()} - Github Repositories - OpenAI"
...
if __name__ == "__main__":
    ...
    # models_names = input("Element to add in models name : ")

    # result_files_name = input("Name to give to result file (evaluation) : ")
