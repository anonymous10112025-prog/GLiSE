...
def get_measures_from_embeddings(all_embeddings, binary_flags):
    only_cosine_distances = [] # 2 dimension vectors -> cosine distances

    only_euclidean_distances = [] # 2 dimension vectors -> euclidean distances

    only_l1_distances = [] # 2 dimension vectors -> L1 distances

    cos_and_euclidean_distances = [] # 4 dimension vectors -> Cosine + euclidean distances

    only_distances = [] # 6 dimension vectors -> cosine, euclidean, L1 distances
    ...
    for flag in binary_flags:
        ...
        # Differences
        differences = difference_vector(search_intent_embedding, title_embedding).tolist() + difference_vector(search_intent_embedding, snippet_embedding).tolist()

        difference_vectors.append(differences)

        # Overlap / Product
        overlap_products = overlap_product_vector(search_intent_embedding, title_embedding).tolist() + overlap_product_vector(search_intent_embedding, snippet_embedding).tolist()
...
def prepare_infos_testing_and_training(the_dataset, dimensions, embedding_model, tokenizer_model):
    ...
    for issue_infos in the_dataset:
        texts_to_embed.append(safe_non_empty_string_field_access("search_intent", issue_infos))
        texts_to_embed.append(safe_non_empty_string_field_access("title", issue_infos))
        # texts_to_embed.append(clean_markdown(safe_non_empty_string_field_access("snippet", issue_infos)))
        texts_to_embed.append(safe_non_empty_string_field_access("snippet", issue_infos))

        relevance_grade = issue_infos["grade"]
...
def llm_binary_decide(search_intent: str, title: str, snippet: str, previous_decision: float) -> bool:
    """
...
Your only job is to decide whether a GitHub issue is relevant
...
Determine whether the Github Issue appears relevant to the intent.
...
def test_gpt_4o_perf(the_test_dataset, nbr_iter = 5):
    ...
    for _ in range(nbr_iter):
        ...
        for issue_infos in the_test_dataset:
            list_decisions.append(llm_binary_decide(
                search_intent=safe_non_empty_string_field_access("search_intent", issue_infos),
                title=safe_non_empty_string_field_access("title", issue_infos),
                # snippet=clean_markdown(safe_non_empty_string_field_access("snippet", issue_infos)),
                snippet=safe_non_empty_string_field_access("snippet", issue_infos),
...
def run_models_training_testing_pipeline_github_issues(train_dataset, test_dataset, add_to_folder_name):

    root_folder = f"{str(add_to_folder_name).strip()} - Github Issues - OpenAI"
