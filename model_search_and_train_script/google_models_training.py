...
def get_measures_from_embeddings(all_embeddings, binary_flags):
    only_cosine_distances = [] # 6 dimension vectors -> cosine distances

    only_euclidean_distances = [] # 6 dimension vectors -> euclidean distances

    only_l1_distances = [] # 6 dimension vectors -> L1 distances

    cos_and_euclidean_distances = [] # 12 dimension vectors -> Cosine + euclidean distances

    only_distances = [] # 18 dimension vectors -> cosine, euclidean, L1 distances
    ...
    for flag in binary_flags:
        ...
        # texts_to_embed.append(safe_non_empty_string_field_access("meta_desc", google_res_infos))
        # texts_to_embed.append(safe_non_empty_string_field_access("schema_desc", google_res_infos))
        # texts_to_embed.append(safe_non_empty_string_field_access("url", google_res_infos))

        # Cosine Distances
        cos_distances = [
            cosine_distance(search_intent_embedding, title_embedding),
            cosine_distance(search_intent_embedding, snippet_embedding),
            cosine_distance(search_intent_embedding, html_snippet_embedding),
            cosine_distance(search_intent_embedding, meta_desc_embedding),
            cosine_distance(search_intent_embedding, schema_desc_embedding),
        ...
        # Euclidean Distances
        euclidean_distances = [
            euclidean_distance(search_intent_embedding, title_embedding),
            euclidean_distance(search_intent_embedding, snippet_embedding),
            euclidean_distance(search_intent_embedding, html_snippet_embedding),
            euclidean_distance(search_intent_embedding, meta_desc_embedding),
            euclidean_distance(search_intent_embedding, schema_desc_embedding),
        ...
        # L1 Distances
        l1_distances = [
            l1_distance(search_intent_embedding, title_embedding),
            l1_distance(search_intent_embedding, snippet_embedding),
            l1_distance(search_intent_embedding, html_snippet_embedding),
            l1_distance(search_intent_embedding, meta_desc_embedding),
            l1_distance(search_intent_embedding, schema_desc_embedding),
        ...
        # Differences
        differences = (
            difference_vector(search_intent_embedding, title_embedding).tolist() 
            + difference_vector(search_intent_embedding, snippet_embedding).tolist() 
            + difference_vector(search_intent_embedding, html_snippet_embedding).tolist()
            + difference_vector(search_intent_embedding, meta_desc_embedding).tolist()
            + difference_vector(search_intent_embedding, schema_desc_embedding).tolist()
            + difference_vector(search_intent_embedding, url_embedding).tolist()
        )
        ...
        # Overlap / Product
        overlap_products = (
            overlap_product_vector(search_intent_embedding, title_embedding).tolist() 
            + overlap_product_vector(search_intent_embedding, snippet_embedding).tolist() 
            + overlap_product_vector(search_intent_embedding, html_snippet_embedding).tolist()
            + overlap_product_vector(search_intent_embedding, meta_desc_embedding).tolist()
            + overlap_product_vector(search_intent_embedding, schema_desc_embedding).tolist()
            + overlap_product_vector(search_intent_embedding, url_embedding).tolist()
        )
...
def prepare_infos_testing_and_training(the_dataset, dimensions, embedding_model, tokenizer_model):
    ...
    for google_res_infos in the_dataset:
        texts_to_embed.append(safe_non_empty_string_field_access("search_intent", google_res_infos))
        texts_to_embed.append(safe_non_empty_string_field_access("title", google_res_infos))
        texts_to_embed.append(safe_non_empty_string_field_access("snippet", google_res_infos))
        texts_to_embed.append(clean_html_for_embedding(safe_non_empty_string_field_access("html_snippet", google_res_infos)))
        texts_to_embed.append(safe_non_empty_string_field_access("meta_desc", google_res_infos))
        texts_to_embed.append(safe_non_empty_string_field_access("schema_desc", google_res_infos))
        texts_to_embed.append(safe_non_empty_string_field_access("url", google_res_infos))

        # relevance_grade = google_res_infos["grade"]
        relevance_grade = google_res_infos["Grade"]
...
def llm_binary_decide(search_intent: str, title: str, snippet: str, html_snippet: str, meta_desc: str, schema_desc: str, url: str, previous_decision: float) -> bool:
    """
...
Your only job is to decide whether a google search result is relevant
...
Determine whether the google search result appears relevant to the intent.
...
def test_gpt_4o_perf(the_test_dataset, nbr_iter = 5):
    ...
    for _ in range(nbr_iter):
        ...
        for google_res_infos in the_test_dataset:
            list_decisions.append(llm_binary_decide(
                search_intent=safe_non_empty_string_field_access("search_intent", google_res_infos),
                title=safe_non_empty_string_field_access("title", google_res_infos),
                snippet=safe_non_empty_string_field_access("snippet", google_res_infos),
                html_snippet=clean_html_for_embedding(safe_non_empty_string_field_access("html_snippet", google_res_infos)),
                meta_desc=safe_non_empty_string_field_access("meta_desc", google_res_infos),
                schema_desc=safe_non_empty_string_field_access("schema_desc", google_res_infos),
                url=safe_non_empty_string_field_access("url", google_res_infos),
...
def run_models_training_testing_pipeline_google_cse(train_dataset, test_dataset, add_to_folder_name):

    root_folder = f"{str(add_to_folder_name).strip()} - Google CSE Results - OpenAI"
